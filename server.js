...
// تفاصيل مستخدم
app.get('/users/:email', requireFounder, (req, res) => {
...
app.post('/users/role', requireFounder, (req, res) => {
    ...
    if (role === "admin" && user.role !== "admin") {
        user.promotedAt = new Date().toLocaleString();
    }

    user.role = role;
    saveData(usersPath, users);
    addLog("تغيير دور مستخدم", email, req.session.user);

    res.redirect('/users');
});

// منح / إزالة صلاحية الإدارة
app.post('/users/toggle-permission', requireFounder, (req, res) => {
    ...
    saveData(usersPath, users);
    addLog("تعديل صلاحيات مستخدم", email, req.session.user);
...
app.post('/users/change-name', requireFounder, (req, res) => {
    ...
    if (!user || user.role === "founder" || !newName || !newName.trim()) {
        return res.redirect('/users');
    }

    user.name = newName.trim();
    saveData(usersPath, users);
    addLog("تغيير اسم مستخدم", email, req.session.user);
